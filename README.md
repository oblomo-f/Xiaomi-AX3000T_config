...
В Web-интерфейсе период очистки выбирается выпадающим меню: **1, 2, 3, 4 или 5 дней**, либо **«Ввести вручную»** для любого количества дней. В UCI параметр `rotate_seconds` по-прежнему хранится в секундах.

### Проверка наличия Podkop перед установкой

Перед установкой Watchdog выполняется проверка наличия сервиса **Podkop** (`/etc/init.d/podkop`).
Если Podkop не установлен, установка Watchdog прекращается и выводится сообщение:

> Ошибка: Podkop не установлен.
> Сначала установите Podkop, затем установите Podkop Watchdog.

Watchdog не устанавливается на роутер без Podkop, поскольку его основная задача — контролировать интернет-соединение и при необходимости перезапускать Podkop.


### Статус Podkop в Web-интерфейсе

Рядом со статусом Watchdog отображается отдельный статус: **Podkop: УСТАНОВЛЕН** или **Podkop: НЕ УСТАНОВЛЕН**. Проверка выполняется по наличию исполняемого `/etc/init.d/podkop`.


### Статус Podkop

Web-интерфейс показывает три состояния: **Podkop: Установлен и работает.**, **Podkop: Установлен, но не работает.** или **Podkop: Не установлен**. Работа определяется по фактическому процессу `sing-box`, а не только по наличию init-скрипта.
